...
class CityRandomizer extends React.PureComponent {
  ...
  getData = () => {
    fetch("https://gp-js-test.herokuapp.com/api")
      ...
      .then(data => {
        this.setState(
          { adjectives: data.adjectives, cities: data.cities },
          () => {
            this.setMaxValueOfPairs();
          }
        );
      });
  };
  ...
  setMaxValueOfPairs = () => {
    this.setState({
      maxValueGeneratedPairs: (
        (this.state.cities.length + this.state.adjectives.length) /
        2
      ).toFixed(0)
    });
  };
  ...
  isCountOfPairsValid = () => {
    return (
      this.state.valueOfGeneratedPairs + 1 < this.state.maxValueGeneratedPairs
    );
  };
  ...
  appendNewPair = (adjective, city) => {
    ...
    this.setState({
      ...
      valueOfGeneratedPairs: this.state.valueOfGeneratedPairs + 1,
      lastGeneratedPair: newPair,
      isBtnEnable: this.isCountOfPairsValid() ? true : false
    });
  };
  ...
  render() {
    return (
      <div className="city-randomizer">
        <GetDataBtn
          isBtnEnable={this.state.isBtnEnable}
          action={this.generatePair}
        />
        <LastGeneratedPair lastPair={this.state.lastGeneratedPair} />
        <p>
          {this.state.valueOfGeneratedPairs}/{this.state.maxValueGeneratedPairs}{" "}
          pairs
        </p>
