...
Cache.prototype.addToCache = function(funcName, arg, result) {
  ...
  if (
    Object.keys(this.resultsCache[funcName]).length > this.maxOfChacheResults
  ) {
    delete this.resultsCache[funcName][
      Object.keys(this.resultsCache[funcName])[0]
    ];
  }
};
...
Cache.prototype.searchInCache = function(funcName, arg) {
  if (funcName in this.resultsCache) {
    if (arg in this.resultsCache[funcName])
      return this.resultsCache[funcName][arg];
    return false;
  } else {
    return false;
  }
