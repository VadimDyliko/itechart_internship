    ...
    if (this.state.emailIdentity && this.state.loginIdentity) {
      ...
      fetch("/singup", {
        ...
        if (res.status === 200) {
          ...
          this.props.dispatch(setModal({isShow: true, modalTitle: 'Registration faild', modalText: 'User is registred with such login or email'}))
          throw new Error('User is registred with such login or email');
        } else {
          this.props.dispatch(setModal({isShow: true, modalTitle: 'Registration faild', modalText: 'Set valid information'}))
    ...
    return (<form className={singUpFormClassNames} onSubmit={this.submitHandler}>
      ...
      <div className="input-group mb-3">
        ...
        <div className="custom-file">
          ...
          <label className="custom-file-label" forhtml="inputFileProfileImage">{this.state.profilePictureName}</label>
        </div>
      </div>
      <div className="form-group">
        <p>Email address</p>
